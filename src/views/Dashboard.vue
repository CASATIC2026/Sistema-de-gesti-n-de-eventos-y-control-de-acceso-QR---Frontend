...
const exportEventSummaryToExcel = async () => {
  ...
  for (const evt of eventsList.value) {
    try {
      // Traer todos los tickets del evento (sin paginación, máximo 100)
      const res = await api.get(`/ticket/event/${evt.id}?pageSize=100`)
      ...
      // Obtener datos de perfil de los usuarios (emails de tickets)
      const emails = tickets.map(t => t.userEmail).filter(Boolean)

      // Buscar info de perfil de cada usuario
      let ages = []
      let genders = { Male: 0, Female: 0, Other: 0, PreferNotToSay: 0, SinDato: 0 }

      try {
        // Llamada al endpoint de estadísticas del evento que ya incluye datos de perfil
        const statsRes = await api.get(`/event/${evt.id}/stats`)
        const s = statsRes.data.data
        console.log("STATS", s)
        console.log("GENDER", s.genderDistribution)

        // Si el backend ya devuelve stats de perfil, úsalas
        if (s?.genderDistribution) {
          genders = s.genderDistribution
        }
        if (s?.avgAge) {
          ages = [s.avgAge]
        }
      } catch {
        // Si no hay stats de perfil, los campos quedan vacíos
      }

      const avgAge = ages.length ? Math.round(ages[0]) : 'Sin dato'
      const total = Object.values(genders).reduce((a, b) => a + b, 0)
      const pct = (val) => {
        const safeTotal = total || 0
        const safeVal = val || 0
        return safeTotal > 0 ? ((safeVal / safeTotal) * 100).toFixed(1) + '%' : '0%'
      }
      ...
      data.push({
        ...
        'Duración\n(horas)': durationHours,
        'Capacidad máx.': evt.maxCapacity,
        'Total registrados': tickets.length,
        'Registrados antes de la fecha': registeredBefore,
        'Registrados el día del Evento': registeredDay,
        'Ingresaron al Evento(QR)': checkedIn,
        'No asistieron': notCheckedIn,
        '% Asistencia': tickets.length
          ? ((checkedIn / tickets.length) * 100).toFixed(1) + '%'
          : '0%',
        'Edad\npromedio': avgAge,
        '% Masculino': pct(genders.male),
        '% Femenino': pct(genders.female),
        '% Otro': pct(genders.other),
        '% Prefiere no decir': pct(genders.preferNotToSay),
        '% Sin dato género': pct(genders.sinDato)
      })
  ...
  { wch: 22 }, // Registrados antes
  { wch: 22 }, // Registrados día evento
  { wch: 20 }, // Ingresaron QR
  { wch: 15 }, // No asistieron
  { wch: 15 }, // % Asistencia
  { wch: 15 }, // Edad promedio
  { wch: 15 }, // Masculino
  { wch: 15 }, // Femenino
  { wch: 15 }, // Otro
  { wch: 20 }, // Prefiere no decir
  { wch: 18 }  // Sin dato género
]
...
const exportEventDetailToExcel = async () => {
  ...
  // Helper: etiqueta de género
  const genderLabel = (g) => {
    const map = { Male: 'Masculino', Female: 'Femenino', Other: 'Otro', PreferNotToSay: 'Prefiere no decir' }
    return map[g] || 'Sin dato'
  }

  // Obtener perfiles de usuarios únicos por email
  // El endpoint GET /user/me solo retorna el usuario actual (admin),
  // así que enriquecemos lo que tengamos. Si en el futuro hay un endpoint
  // admin para buscar usuarios, se puede usar aquí.
  // Por ahora, incluimos los campos vacíos y lo dejamos preparado.
  ...
  const data = allTickets.value.map(t => {
    ...
    // Estos campos se llenarán cuando el backend exponga un endpoint
    // para buscar perfiles por email (p.ej. GET /user/profile?email=...)
    const age = t.userAge !== undefined ? t.userAge : (t.birthDate ? calcAge(t.birthDate) : 'Sin dato')
    const gender = t.userGender !== undefined ? genderLabel(t.userGender) : (t.gender ? genderLabel(t.gender) : 'Sin dato')

    return {
      Email: t.userEmail,
      Evento: event?.name || 'N/A',
      'Fecha evento': startDate ? startDate.toLocaleString() : 'N/A',
      'Fecha registro': registeredAt.toLocaleString(),
      'Registrado antes del evento': startDate ? registeredAt < startDate ? 'Sí' : 'No' : 'N/A',
...
const submitEvent = async () => {
  try {
    ...
    const payload = {
      ...
      isActive:
        eventForm.value.isActive !== undefined
          ? eventForm.value.isActive
          : true
    }
    ...
    // Esperar para que el usuario vea el toast
    setTimeout(async () => {
      discardDraft()
      await loadEvents()
      await fetchEventsAndStats()
  ...
  <div class="bg-surface text-on-surface min-h-screen font-body overflow-hidden flex flex-col">
    ...
    <main
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-0 mb-8 lg:mb-16">
        ...
        <div class="hidden sm:flex items-center gap-3">
          ...
          <div class="h-12 w-12 lg:h-14 lg:w-14 rounded-full bg-surface-container-highest overflow-hidden">
            <img alt="Admin Avatar" class="w-full h-full object-cover"
              src="https://st2.depositphotos.com/3554337/11574/i/950/depositphotos_115748516-stock-photo-golden-profile-icon.jpg" />
          </div>
        </div>
      </header>
      ...
      <section v-if="currentSection === 'events'" class="bg-surface-container-low rounded-xl overflow-hidden">
        <div class="relative">
  <!-- Encabezado con gradiente y efecto -->
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-surface-container-low to-surface-container mb-6">
    ...
    <div class="relative px-6 py-8 lg:px-8 lg:py-0">
      <!-- Título -->
      <div class="mb-6">
     
        <h2 class="text-3xl sm:text-4xl lg:text-4xl font-bold font-headline tracking-tight bg-gradient-to-r from-on-surface to-on-surface/70 bg-clip-text text-black">
          Listado de Eventos
        </h2>
        <div class="h-1 w-20 bg-gradient-to-r from-primary to-primary/20 rounded-full mt-3"></div>
      </div>
      
      <!-- Barra de filtros + Botón Scanner -->
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <div class="relative w-full sm:w-72">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-secondary/60 text-lg">search</span>
            <input 
              v-model="searchQuery"
              type="text"
              placeholder="Buscar eventos..."
              class="w-full pl-10 pr-4 py-2.5 bg-surface-container-lowest border border-outline-variant/20 rounded-xl text-sm font-body text-on-surface placeholder:text-secondary/50 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all"
            />
          </div>
          ...
          <select 
            ...
            class="w-full sm:w-40 bg-surface-container-lowest px-4 py-2.5 rounded-xl text-xs font-label uppercase tracking-wider text-on-surface border border-outline-variant/20 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all cursor-pointer"
          >
        ...
        <router-link 
          to="/scanner"
          class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-primary to-primary/80 text-white rounded-xl font-semibold text-sm shadow-md hover:shadow-lg transition-all hover:scale-105 group"
        >
          <span class="material-symbols-outlined text-base group-hover:animate-pulse">qr_code_scanner</span>
